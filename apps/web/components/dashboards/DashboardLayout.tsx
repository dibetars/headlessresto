  ...
  const SidebarContent = () => (
    <div className="p-6 h-full flex flex-col">
      ...
      <nav className="flex-1 overflow-y-auto no-scrollbar space-y-0.5">
        ...
        {/* Manager / Owner — MVP: Menu, Orders, KDS, Storefront */}
        {(role === 'owner' || role === 'manager') && (
          <NavGroup label="Storefront">
            <NavItem icon={<Globe className="w-4 h-4" />} label="Storefront" href="/dashboard/storefront" active={pathname === '/dashboard/storefront'} onClick={() => setSidebarOpen(false)} />
            {feat('menu') && <NavItem icon={<Store className="w-4 h-4" />} label="Menu" href="/dashboard/menu" active={pathname === '/dashboard/menu'} onClick={() => setSidebarOpen(false)} />}
          </NavGroup>
        )}
